...
class VolumetricPositionEncoding(nn.Module):
    ...
    def pe(self, XYZ):
        ...
        if npoint == 0:
            print(f"[VolumetricPositionEncoding - outlier rejection] Warning: input XYZ has no points, returning zeros. Shape: {XYZ.shape}")
        ...
        # Apply mask after PE calculation
        position_code = position_code * mask

        #print("mask sum:", mask.sum())  # Should be number of valid points
        if torch.isnan(position_code).any():
